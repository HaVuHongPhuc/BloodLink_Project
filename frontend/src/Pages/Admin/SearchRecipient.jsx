...
const SearchRecipient = () => {
  ...
  const fetchAllRecipients = async (searchKeyword = keyword, fromDate = startDate, toDate = endDate) => {
    try {
      ...
      const res = await fetch(`http://localhost:5000/api/admin/tra-cuu-nguoi-nhan?${params.toString()}`, {
  ...
  const handleSearch = () => {
    fetchAllRecipients(keyword, startDate, endDate);
  };
          ...
          <table className="w-full border-collapse text-sm">
            <thead>
              <tr className="bg-gray-100">
                ...
                <th className="p-2 text-left">Nhóm máu cần</th>
